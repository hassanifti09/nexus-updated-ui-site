...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    // Render at lower resolution for performance, CSS will scale up
    ...
    const fragmentShader = `
      ...
      // Apply layered wave deformation to position (unrolled for GPU performance)
      vec3 applyWaveDeformation(vec3 pos, float timeOffset) {
        vec3 deformed = pos;

        // Iteration 0
        deformed.xz *= rot(0.4);
        deformed += cos(deformed.zxy * 1.0 - timeOffset * 0.0);

        // Iteration 1
        deformed.xz *= rot(0.4);
        deformed += cos(deformed.zxy * 2.0 - timeOffset * 2.0) * 0.5;

        // Iteration 2
        deformed.xz *= rot(0.4);
        deformed += cos(deformed.zxy * 4.0 - timeOffset * 4.0) * 0.25;
    ...
    const handleResize = () => {
      ...
      resizeTimeout = window.setTimeout(() => {
        ...
        const newWidth = Math.floor(containerRef.current.clientWidth * renderScale);
        const newHeight = Math.floor(containerRef.current.clientHeight * renderScale);
